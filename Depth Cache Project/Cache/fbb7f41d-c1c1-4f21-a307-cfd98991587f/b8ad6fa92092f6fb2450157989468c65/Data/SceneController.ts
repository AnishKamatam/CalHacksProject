...
export class SceneController extends BaseScriptComponent {
  ...
  @input
  @hint("Handles Spanish translation of labels (optional)")
  @allowUndefined
  ...
  private sendToGemini(
    ...
    this.gemini.makeGeminiRequest(cameraFrame, text, (response) => {
      ...
      // Translate points if translation manager is available
      var processedPoints = response.points;
      if (this.translationManager) {
        processedPoints = this.translationManager.translatePoints(response.points);
      }
      
      print("GEMINI Points LENGTH: " + processedPoints.length);
      this.responseUI.openResponseBubble(response.aiMessage);
      //create points and labels
      for (var i = 0; i < processedPoints.length; i++) {
        var pointObj = processedPoints[i];
        if (this.showDebugVisuals) {
