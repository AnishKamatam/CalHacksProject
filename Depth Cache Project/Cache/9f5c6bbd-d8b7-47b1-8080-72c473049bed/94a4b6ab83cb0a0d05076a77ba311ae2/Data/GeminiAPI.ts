...
import { SpanishTranslator } from "./SpanishTranslator";
...
export class GeminiAPI extends BaseScriptComponent {
  @input spanishTranslator: SpanishTranslator | null;
  ...
  private onGeminiResponse(
    ...
    try {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var labelText = data[i].label;
        
        // Translate to Spanish if enabled and translator is available
        if (this.enableSpanishTranslation && this.spanishTranslator) {
          labelText = this.spanishTranslator.translate(data[i].label);
        }
        ...
        var lensStudioPoint = {
          ...
          originalLabel: data[i].label, // Keep original for reference
        };
