...
export class InteractorCursor extends BaseScriptComponent {
  ...
  onAwake(): void {
    ...
    // Only create manipulate line config if material exists and is valid
    try {
      const material = requireAsset("./ManipulateLineMaterial.mat") as Material
      // Validate that the material has a valid mainPass
      if (material && material.mainPass) {
        ...
        print("ManipulateLineMaterial.mat has no valid mainPass")
        this.manipulateLineConfig = null
      }
    } catch (e) {
      print(`Failed to load ManipulateLineMaterial.mat: ${e}`)
      this.manipulateLineConfig = null
    }
