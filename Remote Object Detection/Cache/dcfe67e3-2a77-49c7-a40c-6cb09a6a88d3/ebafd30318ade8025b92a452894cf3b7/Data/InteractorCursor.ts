...
export class InteractorCursor extends BaseScriptComponent {
  private circleVisualConfig: CircleVisualConfig | null = null

  private manipulateLineConfig: LineViewConfig | null = null

  private circleVisual: CircleVisual | null = null
  private circleVisualEnabled = true

  private manipulateLine: LineRenderer | null = null
  ...
  private onCursorUpdate = (viewState: CursorViewState) => {
    ...
    if (viewState.lineEnabled && this.manipulateLine) {
  ...
  onAwake(): void {
    try {
      ...
      print(`InteractorCursor initialization failed: ${e}`)
      // Create minimal config to prevent further errors
      this.circleVisualConfig = null
      this.manipulateLineConfig = null
      return
    }
    // Try to load materials in order of preference
    let material: Material | null = null
    
    // First try the original material
    try {
      const originalMaterial = requireAsset("./ManipulateLineMaterial.mat") as Material
      if (originalMaterial && originalMaterial.mainPass) {
        material = originalMaterial
      }
    } catch (e) {
      print(`ManipulateLineMaterial.mat failed: ${e}`)
    }
    
    // Fallback to Cursor material if original failed
    if (!material) {
      try {
        const fallbackMaterial = requireAsset("./Cursor.mat") as Material
        if (fallbackMaterial && fallbackMaterial.mainPass) {
          material = fallbackMaterial
          print("Using Cursor.mat as fallback for manipulate line")
        }
      } catch (e) {
        print(`Cursor.mat fallback failed: ${e}`)
      }
    }
    
    // Create config only if we have a valid material
    if (material) {
      ...
      print("No valid material found for manipulate line - disabling feature")
      this.manipulateLineConfig = null
    }

    // Only create visuals if configs were successfully created
    if (this.circleVisualConfig) {
      try {
        this.circleVisual = new CircleVisual(this.circleVisualConfig)
      } catch (e) {
        print(`Failed to create CircleVisual: ${e}`)
        this.circleVisual = null
      }
    }

    // Only create manipulate line if config was successfully created
    if (this.manipulateLineConfig) {
      try {
        ...
        print(`Failed to create LineRenderer: ${e}`)
        this.manipulateLine = null
      }
    }
