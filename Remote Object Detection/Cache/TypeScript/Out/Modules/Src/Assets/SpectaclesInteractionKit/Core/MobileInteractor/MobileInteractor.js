...
let MobileInteractor = (() => {
    ...
    var MobileInteractor = _classThis = class extends _classSuper {
        ...
        get distanceToTarget() {
            return (this.indirectTargetProvider?.currentInteractableHitInfo?.hit.distance ??
                null);
        }
        get targetHitPosition() {
            return (this.indirectTargetProvider?.currentInteractableHitInfo?.hit.position ??
                null);
        }
        get targetHitInfo() {
        ...
        updateDragVector() {
            if ((this.currentTrigger & Interactor_1.InteractorTriggerType.Select) !== 0) {
                const touchpadDragVector = this.touchpadDragProvider.getDragVector(this.getTouchpadDragPoint(), this.currentInteractable?.enableInstantDrag ?? null);
                const sixDofDragVector = this.sixDofDragProvider.getDragVector(this.getSixDofDragPoint(), this.currentInteractable?.enableInstantDrag ?? null);
