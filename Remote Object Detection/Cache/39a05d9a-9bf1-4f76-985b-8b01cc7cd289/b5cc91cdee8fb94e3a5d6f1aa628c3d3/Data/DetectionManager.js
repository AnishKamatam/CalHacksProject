...
// Simple auto-detection without complex UI
let autoDetectionStarted = false;

script.createEvent("UpdateEvent").bind(() => {
  // Start auto-detection ONCE after 3 seconds
  if (!autoDetectionStarted && getTime() > 3.0) {
    autoDetectionStarted = true;
    startSimpleDetection();
    debugLog("Auto-detection triggered once - no more repeats");
  }
});

function onStart() {
  // Simple initialization without complex SIK components
  ...
  debugLog("Object Detection initialized - will start automatically in 3 seconds");
  setStatus("Object Detection Ready - Starting soon...");
}

// Simple detection function without complex interaction system
async function startSimpleDetection() {
  if (script.hfApiToken === null || script.hfApiToken === "") {
    debugLog("Error: Hugging Face API token is not set!");
    return;
  }

  debugLog(">>> Starting simple object detection...");
  setStatus("Detecting objects...");

  try {
    await processTexture();
    debugLog("Simple detection completed");
    setStatus("Detection complete! Objects should be visible.");
  } catch (e) {
    debugLog(`Error in simple detection: ${e}`);
    setStatus("Detection failed - check console for details");
  }
}

// Legacy function - no longer used with simple detection
...
async function createDetectionPrefabAsync(detection) {
  try {
    debugLog("Creating simple detection object...");
    const detectionObj = scene.createSceneObject(`Detection_${detection.class_name}`);
    
    // Create a simple text component instead of complex prefab
    const textComponent = detectionObj.createComponent("Component.Text");
    if (!textComponent) {
      throw new Error("Failed to create text component");
    }

    // Set up simple text display with safe properties
    textComponent.text = `${detection.class_name} (${(detection.confidence * 100).toFixed(1)}%)`;
    
    // Set text properties safely
    try {
      textComponent.size = new vec2(200, 50);
      textComponent.horizontalAlignment = HorizontalAlignment.Center;
      textComponent.verticalAlignment = VerticalAlignment.Center;
    } catch (e) {
      debugLog(`Warning: Could not set text properties: ${e}`);
    
    // Normalize coordinates and project to 3D
    const normalizedCenter = new vec2(
      (detection.center_2d[0] + 0.5) / 640,
      1 - (detection.center_2d[1] + 0.5) / 480
    );
    
    debugLog(`Detection: ${detection.class_name} at normalized center: ${normalizedCenter}`);
    
    // Project 2D to 3D world position
    const worldPosition = script.pinholeCapture.captureToWorld(
      normalizedCenter,
      detection.distance * offsetPositioning
    );
    
    // Set world position
    detectionObj.getTransform().setWorldPosition(worldPosition);
    debugLog(`${detection.class_name} positioned at: [${worldPosition.x.toFixed(3)}, ${worldPosition.y.toFixed(3)}, ${worldPosition.z.toFixed(3)}]`);

    // Set text color based on detection color
    if (detection.color && textComponent.textFill) {
      const color = detection.color;
      const newColor = new vec4(
        color[0] / 255.0,
        color[1] / 255.0,
        color[2] / 255.0,
        1.0
      );
      textComponent.textFill.color = newColor;
    }
...
function sendToHuggingFace(encodedImage) {
  ...
  const modelUrl =
    "https://parupupapi-simultaneous-segmented-depth-prediction.hf.space/gradio_api/call/model_selector";
  const confidenceUrl =
    "https://parupupapi-simultaneous-segmented-depth-prediction.hf.space/gradio_api/call/update_confidence_threshold";
  ...
  const apiUrl =
    "https://parupupapi-simultaneous-segmented-depth-prediction.hf.space/gradio_api/call/get_detection_data";
